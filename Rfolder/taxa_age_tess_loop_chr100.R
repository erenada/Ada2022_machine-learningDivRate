...
out_dir <- "/data/schwartzlab/eren/Chapter3/taxa_age_outdir100"
...
for(dataset in list.dirs(input_dir, recursive = F,full.names = F)){
  for(tree in list.files(paste(input_dir,dataset, sep = "/"))){
    ...
    tree_object <- rescale(tree_object, model = "depth", 100)
    ...
    ##model evaluation with Bayesian Factor comparison
